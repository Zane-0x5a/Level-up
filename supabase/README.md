...
- 应用发布不得把尚未确认上线的新增列、约束或 RPC 作为核心流程的必需条件。
- 需要数据库扩展时采用 expand/contract：先上线向后兼容的迁移，确认生产 schema，再启用依赖它的代码。
- 能复用现有稳定契约时优先复用。例如专注记录使用客户端生成的 UUID 作为现有 `id` 主键，以获得幂等性，而不是依赖额外列。
- 每个写入契约必须有旧版生产 schema 测试，确保 migration 延迟不会让用户核心数据无法保存。
